...
module Rbnput
  module Keyboard
    ...
    class KeyCode
      attr_reader :vk, :char, :is_dead

      def initialize(vk: nil, char: nil, is_dead: false, **kwargs)
        @vk = vk
        @char = char
        @is_dead = is_dead
      ...
      def to_s
        if @char
          @is_dead ? "KeyCode(char=#{@char.inspect}, dead)" : @char.inspect
        elsif @vk
          "KeyCode(vk=#{@vk})"
        else
          "KeyCode()"
      ...
      def ==(other)
        ...
        @vk == other.vk && @char == other.char && @is_dead == other.is_dead
      end

      def hash
        [@vk, @char, @is_dead].hash
      end
      ...
      # Create a dead key
      def self.from_dead(char, **kwargs)
        new(char: char, is_dead: true, **kwargs)
      end
    end
